...
async function fetchData(routeName) {
  ...
  try {
    ...
    return data
      .replace(/\$_TITLE_/g, response.data.meta.title)
      .replace(/\$_DESCRIPTION_/g, response.data.meta.description);
  } catch (error) {
    return data
      .replace(/\$_TITLE_/g, 'Error')
      .replace(/\$_DESCRIPTION_/g, 'There was an error on this page.');
  }
}
